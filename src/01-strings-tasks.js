...
function encodeToRot13(str) {
  const upp = 'ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLM'.split('');
  const low = 'abcdefghijklmnopqrstuvwxyzabcdefghijklm'.split('');

  return str.split('').map((val) => {
    if (upp.includes(val)) {
      return upp[upp.indexOf(val) + 13];
    }

    if (low.includes(val)) {
      return low[low.indexOf(val) + 13];
    }
